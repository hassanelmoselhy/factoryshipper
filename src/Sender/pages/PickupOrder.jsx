...
export default function PickupRequestManagement() {
  ...
  // Check if all orders are selected (for indeterminate state if needed)
  const isAllSelected = PendingOrders.length > 0 && selectedOrders.length === PendingOrders.length;
  ...
  useEffect( ()=>{
      ...
      try{
        setLoading(true);
        const res=await fetch('https://stakeexpress.runasp.net/api/Shipments/getShipments?statusFilter=Pending',{
          method: 'GET',
          headers:{
